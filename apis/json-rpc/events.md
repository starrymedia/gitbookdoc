...
* [Cosmos SDK Events](https://docs.cosmos.network/v0.50/core/events)&#x20;
...
It is possible to subscribe to `Events` via Tendermint's [Websocket](https://docs.tendermint.com/v0.34/tendermint-core/subscription.html#subscribing-to-events-via-websocket). This is done by calling the `subscribe` RPC method via Websocket:
...
Uptick also supports the Ethereum [JSON-RPC](https://eth.wiki/json-rpc/API) filters calls to subscribe to [state logs](https://eth.wiki/json-rpc/API#eth\_newfilter), [blocks](https://eth.wiki/json-rpc/API#eth\_newblockfilter) or [pending transactions](https://eth.wiki/json-rpc/API#eth\_newpendingtransactionfilter) changes.
...
Then you can check if the state changes with the [`eth_getFilterChanges`](https://eth.wiki/json-rpc/API#eth\_getfilterchanges) call:
...
```bash
uptickd start --rpc.laddr="tcp://127.0.0.1:26657"
```
...
Since Uptick runs uses Tendermint Core as it's consensus Engine and it's built with the Cosmos SDK framework, it inherits the event format from them. However, in order to support the native Web3 compatibility for websockets of the [Ethereum's PubSubAPI](https://geth.ethereum.org/docs/rpc/pubsub), Uptick needs to cast the Tendermint responses retrieved into the Ethereum types.
